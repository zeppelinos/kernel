// Kernel
export const KERNEL_VERSION = "0.0.1";
export const NEW_VERSION_COST = new web3.BigNumber('2e18');
export const DEVELOPER_FRACTION = new web3.BigNumber(10);
export const ZEPPELIN_ACCOUNT = web3.eth.accounts[2];

// Release
export const ERC721_CONTRACT_NAME = 'ERC721Token';
export const DEVELOPER_ACCOUNT = web3.eth.accounts[2];

// Project Information
export const PROJECT_VERSION = "0.0.1";
export const PROJECT_ACCOUNT = web3.eth.accounts[2];
